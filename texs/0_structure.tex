...
\begin{itemize}
  \item \twrite{Administratives:} Hier befinden sich viele verschiedene
  ...
  \item \twrite{Lehrveranstaltungen:} Der wichtigste Ordner; wird
...
\twrite{Lehrveranstaltungen/Unterlagen\_Dynamik/} alle wichtigen Dateien zum
...
Wie in \figref{fig:formelsammlung} dargestellt, befinden sich im Unterordner 
...
\twrite{Vorlesungsbehelf\_yyyy\_zzzz} der einzig wichtige, da alle anderen nur
...
kommenden Semesters ausgestattet, und nur noch dieser neue Ordner bearbeitet. 
Somit wird sichergestellt, dass immer eine vollständige Version der 
...
In \figref{fig:fsakt} wird der Aufbau des Ordners der aktuellen
...
\begin{tikzpicture}[node distance = 1.5cm, auto]
  ...
  \node [description, right of=cd, node distance=5cm] (wichtigcd)
   {In Kapitel gegliederte .cdr-files};
...
\begin{itemize}
  \item \twrite{Aktuelle\_Lehrveranstaltung:} Dieser Ordner beinhaltet im
    ...
    \twrite{Durchgerechnet\_WSyyzz.tex} und \linebreak
  ...
  \item \twrite{Aufgaben.tex:} Dieses \LaTeX-file dient zur Erstelllung
  ...
  \item \twrite{Basisdaten/crd:} Alle {\tt .cdr}-files. Es gibt hier keine
